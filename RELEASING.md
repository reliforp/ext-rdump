...
4. The `release` workflow checks the version is in sync, validates `package.xml`,
   builds, runs the tests, produces `rdump-X.Y.Z.tgz`, and attaches it to the
   GitHub Release as a convenience source archive.
